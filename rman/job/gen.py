...
class Gen(object):
    ...
    def ts_directory(cls, ts_name):
        ...
        """
        tmp_data = db.session.query(cls.DepartmentModel.name, cls.TaskConfigModel)\
            .join(cls.TaskConfigModel, cls.DepartmentModel.id == cls.TaskConfigModel.department_id)\
            .filter(cls.TaskConfigModel.task_name == ts_name)\
            .first()

        if not tmp_data:
            raise TSetNotFound("not found case named '{}'".format(ts_name))

        ts_data = getattr(tmp_data, cls.TaskConfigModel.__name__)
        department_name = tmp_data.name
        id_named_module = "{1}[{0}]".format(ts_data.id, ts_data.task_name)

        # generate folder
        pj_path = gen_yaml_folder(department_name, id_named_module)

        cls._ts_info.update({
            "project_path": pj_path,
            "project_name": department_name,
            "project_module": id_named_module,
    ...
    def ts_file(cls, ts_info=None):
        ...
        ids = []
        all_cases = json.loads(ts_obj.cases)
        ...
        ts_yaml_path = os.path.join(pj_path, "{0}.yaml".format(ts_obj.task_name))
